...
//6 - Rim
int mode = 1;
GLuint Program1, Program2, Program3, Program4, Program5, Program6, Program7;
GLuint VAO;
//std::vector<GLushort> indices;
GLuint shader_program;
//transform
glm::mat4 model, model1, viewProjection;
glm::mat3 normaltr;
...
void initShader7()
{
	std::string readed = readfile("vertex8.shader");
	const char* vsSource = readed.c_str();

	std::string readed2 = readfile("fragment8.shader");
	const char* fsSource = readed2.c_str();
	...
	Program7 = glCreateProgram();
	glAttachShader(Program7, vShader);
	glAttachShader(Program7, fShader);
	glLinkProgram(Program7);

	int link_ok;
	glGetProgramiv(Program7, GL_LINK_STATUS, &link_ok);
	if (!link_ok)
	{
		std::cout << "(4)error attach shaders \n";
		GLchar infoLog[512];
		GLint size;
		glGetProgramInfoLog(Program7, 512, &size, infoLog);
		std::cout << infoLog;
		return;
	}
...
void texture_toon()
{
	...
	//GLint Unif_tex = glGetUniformLocation(Program7, "myTexture");
